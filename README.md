...
Do you find clicking around Salesforce to be slow and tedious? **Imagine if you
could get to ***any*** page or record with only a few words in your browser!**
Go Router's mission is to make that possible! Configure shortcuts to:
...
or if no exact match is found, immediately land on an ***object-scoped search***
for your query:

![Object-Scoped Search Redirect](docs/images/record_search_scoped.png)

The Go Router Framework allows admins and developers to quickly and easily build
routes straight from the setup menu for any use case in their instance. Routes
are designed to be self-documenting, and can be tied to custom-permissions to
ensure that only the proper users in your organization have access to each
route. Quickly access all self-accessible routes using the `help` route:

![Help command](docs/images/help_command.png)

![Go Router Docs Tab](docs/images/docs_page.png)
...
> is recommended to "install for admins only" or "install for specific profiles"
> in order to ensure that only those profiles get access to the admin-specific
> routes. Afterwards, it is easy to enable access to other users through either
> the provided `Go Router - Basic Access` permission set or manually enabling
> access to the `go` tab and Visualforce page:
>
> <img alt="Manual Profile Enablement: Tab Access" height="50" src="docs/images/profile_enablement.png"> <img alt="Manual Profile Enablement: VF Page Access" height="50" src="docs/images/enable_vf_page.png">
>
> To manually provide access to admin/developer routes, use the `Go Router -
> Admin/Developer Access` permission set.
...
Type your shortcut and "help" into the omnibar to verify everything is
working. For example, if your shortcut was `sf`, type in `sf help`, and you
should land on the `Salesforce Go` page within your Salesforce instance!
...
Once Go Router is installed, use Go Router to quickly navigate to the creation
page!
...
Here is an example to setup a router that makes it easy to install Salesforce
packages. If a package ID is passed in, the route will redirect to the
installation URL. If no input is passed in, it will instead redirect to the
...
query will be defined based on the config, and if the query returns exactly
**one** result, it will route to the corresponding record page. To add more
flexibility to the search, two queries can be defined: a specific query and a
broader "like" query. If the specific query fails, the broader query will be
performed as a second chance to match on the input. If both fail, it will
redirect to the Salesforce search page, scoping the query to the specific object
(this even works for setup objects!).
...
router of an instance. That is, define the logic for a "key-less" route. This
can involve sending the input to a couple of common use cases and falling back
to search. Take the following example:
...
Then define the router definition to bind it to the Apex class. Be sure to check
`Is Default Router?`. It is typically recommended to set the key as `_` for the
default router:
...
Out of the box, Go Router provides functionality for tracking usages of routers.
This allows for admins to understand how much usage the package is getting,
which routes provide the most value, and learn when users are having issues with
routers not matching on the expected input.

**By default, logging is disabled to preserve database space in your org!**

To enable logging, go to the custom settings page and `enable logging` for
designated profiles or users:

![Go command to get to settings](docs/images/go_to_settings.png)

![Enable logging](docs/images/settings_logging_enablement.png)

Then assign yourself the `Go Router - View All Logs` permission set. Then you
can verify logs are being captured by navigating to the object listing page:
...
Go Router will also recognize when you pass in a Salesforce ID and will route to
the proper page to view that record. This is especially helpful for certain
setup objects where passing in a normal ID will result in an error page.
GoRouter is able to recognize a list of setup objects and reroute to the proper
page.

For example, take a DeployRequest. If you try to navigate to `/{deployId}`,
Salesforce will error:

![404 on basic ID navigation](docs/images/404.png)

However, when running it through Go Router (`/apex/go?{deployId}`), you'll land
on the proper page!
